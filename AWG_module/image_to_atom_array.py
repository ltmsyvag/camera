...
def image_process(image, nx, ny, x0, y0, x1, y1, x2, y2, x3, y3, rec_x, rec_y, count_threshold):
    ...
    参数说明：
    rec_x, rec_y -> MATLAB中dx_rec/dy_rec参数，固定为2（生成3×3区域）
...
def test_corrected():
    ...
    # 在已知坐标设置高信号（对应掩模位置）
    test_img[50:53, 60:63] = 1000  # (y,x) = (50,60)
    test_img[103:106, 55:58] = 1500  # (y,x) = (103,55)

    # 调用函数（参数对应MATLAB示例）
    mask, atoms = image_process(
        image=test_img,
        nx=10, ny=10,
        x0=52, y0=41,  # MATLAB中的x0 = x1-1 =53-1
        x1=53, y1=42,  # 基准点1
        x2=113, y2=43,  # X基矢终点
        x3=51, y3=103,  # Y基矢终点
